...
export default function TournamentPage() {
    ...
    useEffect(() => {
        // Mock data logic replaces the supabase fetch for now
        async function getTournament() {
            ...
            if (tournamentData) {
                ...
                if (tournamentData.tournamentparticipants) setParticipants(tournamentData.tournamentparticipants)
                // if (tournamentData.rounds) setRounds(tournamentData.rounds)
            }
        }
                    ...
                    <div className="relative w-full h-[400px] md:h-[500px] rounded-2xl overflow-hidden mb-12 shadow-2xl border border-[#333] group">
                        {tournament?.bannerImageUrl ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-br from-[#262626] to-[#1a1a1a] flex items-center justify-center">
                                <span className="text-gray-600 font-bold text-2xl">No Banner Available</span>
                            </div>
                        )}
                        ...
                        <div className="absolute top-6 right-6 z-20">
                            <span className={`
                                ...
                                ${tournament?.status === 'InProgress' ? 'bg-red-500/80 text-white animate-pulse' : ''}
                                ${tournament?.status === 'Registering' ? 'bg-green-500/80 text-white' : ''}
                                ${tournament?.status === 'Completed' ? 'bg-gray-800/80 text-gray-400' : ''}
                            `}>
                                {tournament?.status === 'InProgress' && <span className="mr-2">●</span>}
                                {tournament?.status}
                            </span>
                        </div>
                            ...
                            <div className="flex flex-col md:flex-row md:items-end gap-6">
                                ...
                                {tournament?.sponsorLogoUrl && (
                                    <div className="mb-2 md:mb-4 h-10 px-4 py-1.5 bg-black/40 rounded-lg border border-white/10 backdrop-blur-md flex items-center hover:bg-black/60 transition-colors cursor-pointer">
                                        <span className="text-[10px] uppercase text-gray-300 mr-2 font-bold tracking-wider">Sponsored by</span>
                                        <img src={tournament.sponsorLogoUrl} alt="Sponsor" className="h-full object-contain max-w-[100px] brightness-125" />
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="grid grid-cols-1 gap-8 mb-12">
                        <div className="bg-[#262626] p-8 rounded-2xl border border-[#333]">
                            ...
                            <div className="space-y-6 text-gray-300">
                                <p className="leading-relaxed text-lg">{tournament?.tournamentDescription}</p>

                                <div className="h-[1px] w-full bg-[#333]"></div>

                                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4">
                                    <DetailItem label="Organiser" value={tournament?.organiserName || 'Unknown'} />
                                    <DetailItem label="Match Type" value={tournament?.matchType} />
                                    <DetailItem label="Format" value={tournament?.tournamentFormat} />
                                    <DetailItem label="Time Control" value={`${tournament?.timeControlBaseMinutes}+${tournament?.timeControlIncrementSeconds}`} />

                                    <DetailItem label="Start Date" value={tournament?.startingTime ? new Date(tournament.startingTime).toLocaleDateString() : 'TBA'} />
                                    <DetailItem label="End Date" value={tournament?.endTime ? new Date(tournament.endTime).toLocaleDateString() : 'TBA'} />
                                    <DetailItem label="Prize Pool" value={tournament?.prizePoolDescription || 'N/A'} highlight />
                                    <DetailItem label="Registered" value={`${participants.length} / ${tournament?.maxPlayers || '∞'}`} />
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col gap-6">
                                ...
                                <div className="flex flex-col lg:flex-row gap-8 justify-between items-start overflow-x-auto pb-8">
                                    {rounds.length > 0 ? (
                                        rounds.map((round, roundIndex) => (
                                            <div key={round.id} className="flex flex-col gap-6 min-w-[300px] w-full lg:w-1/3">
                                                ...
                                                <div className="flex flex-col gap-6 relative">
                                                    ...
                                                    {round.matches && round.matches.map((match: any, matchIndex: number) => (
                                        ...
                                        <div className="text-gray-400 italic">No rounds scheduled yet.</div>
                                    )}
                                </div>
                            </div>
                        )}
                                        ...
                                        >
                                            <div className="w-12 h-12 rounded-full bg-[#333] flex items-center justify-center overflow-hidden border border-[#444] group-hover:border-yellow-500/50 transition-colors">
                                                {p.avatar_url ? (
                                                    ...
                                                    <span className="font-bold text-gray-400 text-lg">{(p.username || p.name || '?')[0]?.toUpperCase()}</span>
                                                )}
                                            </div>
                                            <div className="flex flex-col min-w-0">
                                                <div className="font-semibold text-gray-200 truncate group-hover:text-yellow-400 transition-colors">
                                                    {p.username || p.name || 'Unknown Player'}
                                                </div>
                                                {p.rank && (
      ...
      `}
        ...
        `}>
            ...
            <div className="p-4 flex flex-col gap-3">
                <PlayerRow player={match.player1} isWinner={match.winner === (match.player1?.name || match.player1?.username)} score={match.score?.split(' - ')[0] || '-'} />
                <div className="h-[1px] w-full bg-[#333]"></div>
                <PlayerRow player={match.player2} isWinner={match.winner === (match.player2?.name || match.player2?.username)} score={match.score?.split(' - ')[1] || '-'} />
            </div>
        ...
        <div className={`flex items-center justify-between ${isWinner ? 'text-yellow-400' : 'text-gray-300'}`}>
            <div className="flex items-center gap-3">
                {player?.avatar || player?.avatar_url ? (
                    <img src={player.avatar || player.avatar_url} alt={player.name || player.username} className="w-8 h-8 rounded-full object-cover border border-[#444]" />
                ) : (
...
function DetailItem({ label, value, highlight = false }: { label: string, value: string | undefined | null, highlight?: boolean }) {
    if (!value) return null
    return (
        <div className="flex flex-col gap-1.5">
            <span className="text-xs uppercase tracking-wider text-gray-500 font-bold">{label}</span>
            <span className={`font-medium text-lg ${highlight ? 'text-yellow-400 font-bold' : 'text-gray-200'}`}>
                {value}
            </span>
        </div>
    )
