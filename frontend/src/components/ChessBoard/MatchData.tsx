...
const MatchDataUI = (data: MatchData) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 h-[720px]">
      {/* Game Status Header */}
      <div
        className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${getStatusColor()} border backdrop-blur-sm p-6 transition-all duration-300`}
      >
              ...
              <tbody>
                {moves && movePairs.length > 0 ? (
                  movePairs.map((move: any, index) => (
                    ...
                    >
                      <td className="w-[25%] px-4 py-3 text-zinc-500 font-medium text-sm">
                        {index + 1}
                      </td>
                      <td className="w-[37.5%] px-4 py-3">
