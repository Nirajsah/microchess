...
use crate::rook_attacks_on_the_fly;
use crate::square::Square;
use crate::Bitboard;

/// A function that returns the occupancy mask for a given index and attack mask.
pub fn set_occupancy(index: u32, bits_in_mask: u32, attack_mask: Bitboard) -> Bitboard {
    let mut occupancy: Bitboard = 0u64;
    let mut attack_m = attack_mask;

    for i in 0..bits_in_mask {
        let square = attack_m.trailing_zeros();
        pop_bit(&mut attack_m, square);
...
/// A function to reset a bit in a bitboard.
...
pub fn make_table(magics: &[MagicEntry; 64], p: Piece) -> Vec<BitBoard> {
    let table_size = match p {
        Piece::WhiteBishop => BISHOP_TABLE_SIZE,
        Piece::WhiteRook => ROOK_TABLE_SIZE,
        _ => unreachable!(),
    };
    ...
    for square in 0..64 {
        ...
        loop {
            let moves = match p {
                Piece::WhiteBishop => {
                    bishop_attacks_on_the_fly(Square::usize_to_square(square as usize), blockers)
                }
                Piece::WhiteRook => {
                    rook_attacks_on_the_fly(Square::usize_to_square(square as usize), blockers)
                }
                _ => unreachable!(),
            };
...
//     let magic = &BISHOP_MAGIC[square as usize];
//     BISHOP_ATTACK_TABLES[magic_index(magic, blockers)]
// }

// pub fn magic_rook_moves(square: Square, blockers: Bitboard) -> Bitboard {
//     let magic = &ROOK_MAGIC[square as usize];
//     ROOK_ATTACK_TABLES[magic_index(magic, blockers)]
// }

// pub fn magic_queen_moves(sq: Square, blockers: Bitboard) -> Bitboard {
//     magic_bishop_moves(sq, blockers) | magic_rook_moves(sq, blockers)
// }
